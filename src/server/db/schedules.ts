...
export const schedulesRepo = {
  ...
  create(input: { title: string; prompt: string; cron: string; enabled: boolean; nextRunAt: number | null }): Schedule {
    const ts = now();
    ...
    db()
      .prepare(
        `INSERT INTO schedules (id, title, prompt, cron, conversation_id, enabled, last_run_at, next_run_at, created_at, updated_at)
         VALUES (@id, @title, @prompt, @cron, @conversationId, @enabled, @lastRunAt, @nextRunAt, @createdAt, @updatedAt)`,
      )
  ...
  update(
    id: string,
    patch: { title?: string; prompt?: string; cron?: string; enabled?: boolean; nextRunAt?: number | null },
  ): Schedule | undefined {
    ...
    db()
      .prepare("UPDATE schedules SET title = ?, prompt = ?, cron = ?, enabled = ?, next_run_at = ?, updated_at = ? WHERE id = ?")
      .run(next.title, next.prompt, next.cron, next.enabled ? 1 : 0, next.nextRunAt, next.updatedAt, id);
    return next;
  },
