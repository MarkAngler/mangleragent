...
export function ExternalAgentChatPage() {
  const { id = "" } = useParams();
  ...
  async function send() {
    ...
    let cid = activeId;
    if (!cid) {
      const conv = await post<Conversation>(`/external-agents/${id}/conversations`);
      cid = conv.id;
      setSelectedId(cid);
      void qc.invalidateQueries({ queryKey: ["agent-conversations", id] });
    }
      ...
      <header className="mb-6 flex items-end justify-between gap-6 border-b border-hairline pb-5">
        <div>
          <div className="flex items-center gap-2">
            <Link to="/external-agents">
              <Mono className="hover:text-ink">external agents</Mono>
            </Link>
